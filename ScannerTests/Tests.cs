...
namespace DirScannerTests
{
    public class Tests
    {
        ...
        {
            string path = "C:\\Users\\eugen\\source\\repos\\DirectoryScanner\\DirScannerTests\\testDirs\\TargetTest";
            DirScanner scanner = new DirScanner(10);
            TreeNode creatorRoot = DirCreator.CreateDirWithTarget(path);
            TreeNode scannerRoot = scanner.Scan(path);
            creatorRoot.Resize();
            scannerRoot.Resize();
            TreeCmp(scannerRoot, creatorRoot);
        ...
        {
            string path = "C:\\Users\\eugen";
            DirScanner scanner = new DirScanner(10);
            TreeNode? partTree = null;

            TreeNode fullTree = scanner.Scan(path);
            fullTree.Resize();

            var scanningTask = Task.Run(() => { partTree = scanner.Scan(path); });
            Thread.Sleep(100);
            scanner.Stop();
            scanningTask.Wait();
            partTree!.Resize();

            Assert.InRange(partTree!.size, 0, fullTree.size - 1);
        }
    }
}